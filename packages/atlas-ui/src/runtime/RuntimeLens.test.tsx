...
vi.mock('./Timeline.js', () => ({ Timeline: (props: Record<string, any>) => {
  ...
  return { type: 'div', props: { children: `Timeline selection: ${props.selection === null ? 'none' : `${props.selection?.traceId}:${props.selection?.spanId}`}` } }
} }))
...
function runtimeSpan(traceId: string, spanId: string, name: string, service: string, status: number, start: string, duration: string, relation: RuntimeTraceBundle['spans'][number]['relation'], parentSpanId?: string): RuntimeTraceBundle['spans'][number] {
  return {
    ...
    links: traceId === TRACE_ONE && spanId === 'child'
      ? [{ traceId: TRACE_TWO, spanId: 'child', flags: 1, attributes: [], droppedAttributesCount: '0' }]
      : [],
    droppedLinksCount: '0', status: { code: status },
...
function runtimeBundle(): RuntimeTraceBundle {
  return {
    ...
    traces: [
      ...
      { traceId: TRACE_TWO, startTimeUnixNano: '9000', endTimeUnixNano: '9100', spanIds: ['child'], rootSpanIds: ['child'], orphanSpanIds: [], cycleBreakSpanIds: [], hotPathSpanIds: ['child'], errorPaths: [] },
    ],
    spans: [
      ...
      runtimeSpan(TRACE_TWO, 'child', 'health', 'api', 1, '9000', '100', 'root'),
    ],
    report: {
...
describe('RuntimeLens', () => {
  ...
  it('synchronizes an explicit span selection across rail, timeline, and details', () => {
    const first = render()
    expect(harness.state.timelineProps?.selection).toBeNull()
    button(first.elements, 'Select database span child in call tree')?.props.onClick()

    const selected = render()
    expect(harness.state.timelineProps?.selection).toEqual({ traceId: TRACE_ONE, spanId: 'child' })
    expect(text(selected.value)).toContain('Span ID: child')
    expect(text(selected.value)).toContain('Selected span child: database.')

    harness.state.timelineProps?.onSelectSpan({ traceId: TRACE_ONE, spanId: 'root' })
    const canvasSelected = render()
    expect(harness.state.timelineProps?.selection).toEqual({ traceId: TRACE_ONE, spanId: 'root' })
  ...
  it('shows import-wide completeness notices when no trace is available', () => {
    const empty = runtimeBundle()
    empty.traces = []
    empty.spans = []
    empty.report = { ...empty.report, traces: 0, spans: 0 }
    const content = text(render(empty).value)
    expect(content).toContain('No imported runtime traces are available.')
    expect(content).toContain('Sampling: 4 sampled spans; 1 unsampled span.')
    expect(content).toContain('OTLP dropped attributes: 4')
    expect(content).toContain('OTLP dropped events: 5')
    expect(content).toContain('OTLP dropped links: 6')
  ...
  it('does not carry a duplicate span ID into a replacement bundle', () => {
    const initial = runtimeBundle()
    ...
    const replacement = runtimeBundle()
    replacement.traces = [replacement.traces[1]!]
    replacement.spans = replacement.spans.filter((span) => span.traceId === TRACE_TWO)
    render(replacement)
    const settled = render(replacement)
    expect(harness.state.timelineProps?.selection).toBeNull()
    expect(text(settled.value)).toContain(`Trace ID: ${TRACE_TWO}`)
    expect(text(settled.value)).toContain('Select a recorded span to inspect its exact evidence.')
  })

  it('navigates and selects an exact visible cross-trace link', () => {
    const first = render()
    button(first.elements, 'Select database span child in call tree')?.props.onClick()
    const selected = render()
    button(selected.elements, 'Links')?.props.onClick()
    const links = render()
    button(links.elements, 'Select linked span')?.props.onClick()
    const target = render()
    expect(text(target.value)).toContain(`Trace ID: ${TRACE_TWO}`)
    expect(harness.state.timelineProps?.selection).toEqual({ traceId: TRACE_TWO, spanId: 'child' })
    button(target.elements, 'Span details')?.props.onClick()
    expect(text(render().value)).toContain('Span name: health')
  })
})
